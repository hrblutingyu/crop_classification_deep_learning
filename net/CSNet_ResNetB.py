...
def convolution_block_B(X, f, filters, stage, block, s=2):
    ...
    if stage==5:
        ...
        X_shortcut = Conv2D(F3, (1, 1), strides=(1, 1), name=conv_name_base + '3',
                            kernel_initializer='glorot_uniform')(X_shortcut)
        print(X_shortcut.shape)
        X_shortcut = BatchNormalization(axis=3, name=bn_name_base + '1')(X_shortcut)
        X_shortcut = Conv2D(F3, (3, 3), strides=(2, 2), name=conv_name_base + '4',padding='same',
                            kernel_initializer='glorot_uniform')(X_shortcut)
        print(X_shortcut.shape)
        X_shortcut = BatchNormalization(axis=3, name=bn_name_base + '2')(X_shortcut)
        X_shortcut = Conv2D(F3, (1, 1), strides=(1, 1), name=conv_name_base + '5',
                            kernel_initializer='glorot_uniform')(X_shortcut)
        print(X_shortcut.shape)
        X_shortcut = BatchNormalization(axis=3, name=bn_name_base + '6')(X_shortcut)


    else:
        X_shortcut = Conv2D(F3, (1, 1), strides=(s, s), name=conv_name_base + '3',
                            kernel_initializer='glorot_uniform')(X_shortcut)
        X_shortcut = BatchNormalization(axis=3, name=bn_name_base + '2')(X_shortcut)
...
# 论文中的第二个网络,backbone为ResNetB-------------------------
# 分类结果4个类别，三种作物 + 背景-------------------------------
# input_shape:如果是rgb数据则为(256, 256, 3),如果是添加了NDVI,则为(256, 256, 4)

def FCSNetResNetB(input_shape=(256, 256, 3), classes=4,lr_init=5e-4):
    ...
    model.compile(optimizer=Adam(lr=lr_init), loss='categorical_crossentropy',
